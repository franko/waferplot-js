...
float test_data[] = {
376.3209537418,0,0,
384.583909461,63.0678207532,30.3718617382,
381.3958098075,5.231106551,69.8042658027,
372.9235787143,-57.8367142021,39.4324040645,
367.1121490323,-63.0678207532,-30.3718617382,
370.5197916019,-5.231106551,-69.8042658027,
379.3786581587,57.8367142021,-39.4324040645,
384.2670814036,126.1356415063,-60.7437234765,
391.9322494157,135.6708200721,34.5460356766,
392.2859726793,81.7241141129,113.6713208002,
385.6958915057,-10.4622131021,139.6085316054,
372.642208409,-97.7531545321,100.2213588964,
362.4801647734,-139.3043085512,13.9394985234,
358.9577353854,-115.6734284042,-78.8648081289,
363.7251253855,-37.91766554,-134.767394573,
373.6230185516,57.5801944383,-127.6108193236,
383.4709868637,126.1356415063,-60.7437234765,
};
...
static void TEST_calc_tps()
{
  std::vector< Vec > test_points;

  const float XY_NORM_FACTOR = 1.0;
  for (int i = 0; i < 17; i++) {
    test_points.push_back(Vec(test_data[3*i+1]/XY_NORM_FACTOR, test_data[3*i], test_data[3*i+2]/XY_NORM_FACTOR));
  }

  unsigned p = test_points.size();
  ...
  for ( unsigned i=0; i<p; ++i )
  {
    ...
    {
      Vec pt_i = test_points[i];
      Vec pt_j = test_points[j];
      pt_i.y = pt_j.y = 0;
  ...
  const double my_regularization = 0.05;
  ...
  for ( unsigned i=0; i<p; ++i )
  {
    ...
    mtx_l(i,i) = mtx_orig_k(i,i) =
      my_regularization * (a*a);

    // P (p x 3, upper right)
    mtx_l(i, p+0) = 1.0;
    mtx_l(i, p+1) = test_points[i].x;
    mtx_l(i, p+2) = test_points[i].z;

    // P transposed (3 x p, bottom left)
    mtx_l(p+0, i) = 1.0;
    mtx_l(p+1, i) = test_points[i].x;
    mtx_l(p+2, i) = test_points[i].z;
  }
  // O (3 x 3, lower right)
  ...
  for (int j = 0; j < 17; j++) {
    float x = test_points[j].x, z = test_points[j].z;
  // Interpolate grid heights
      ...
      fprintf(stderr, ">> %d %f %f %f EXPECTED: %f\n", j, x, z, h, test_points[j].y);
  }
}
